...
\begin{itemize}
    \item Where did we acquire the dataset(s) from?
    \item What procedures / techniques were applied on the dataset to clean it?

The raw dataset came as a JSON file, which was not directly usable for training. The JSON was nested, so each mix had fields like the mix id, tracklist, track titles, artists, timestamps, and the order of transitions. The first thing we did was parse through this structure and pull out only the fields that were actually relevant to our task, then flatten everything into a table with one row per track per mix.

After that, we cleaned it up by removing any rows where important information was missing. If a track was missing an identifier or could not be matched to its audio features later, we removed it. We also checked for duplicate tracks, since the same song sometimes appeared with slightly different titles, artist names, or formatting in the raw data.

Once the file was cleaned and converted into a csv file, it was so much easier to work with.

    \item What considerations were contemplated during the process of dataset selection and cleaning?
\end{itemize}

\textbf{Important Statistics to Add}

\begin{itemize}
    \item Number of unique tracks with extracted features
    \item Number of positive pairs = (3,721)
    \item Number of total pairs after negative sampling (7,442)
    \item Train/test split ratio (80/20 by mix id)
    \item Train pair count (5,940) and test pair count (1,502)
\end{itemize}
...
After consolidating the songs and organizing our dataset, the next step was to extract acoustic features from each song to form a tabular dataset that could be used to predict our target. We used the Librosa Python package \citep{mcfee2015librosa} to extract all available features the package offers, while excluding redundant feature families to avoid correlation. The features captured three musical dimensions central to DJ practice: tempo (beats per minute, BPM), energy (RMS amplitude), and rhythmic structure (tempogram, tempogram ratio, fourier tempogram). Additional features included spectral characteristics (centroid, bandwidth, contrast, flatness), harmonic content (chroma and tonnetz), and timbral content (MFCCs).
...
Given a song $A$ with tempo $T_A$ and a song $B$ with tempo $T_B$, minimizing each term of \cref{harmonicBPM} leads to a perfect superposition of the two tracks under any of the three DJ-relevant ratios. The full feature extraction code is available at \url{[your-github-repo-here]}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{ScoreHV.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\linewidth]{MeanScoreHV.png}
