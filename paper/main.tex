...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/harmonic_bpm_random.png}
        \caption{Random negative sampling. Positive pairs have a mean harmonic distance of 4.90 BPM (median 2.45) compared to 11.45 BPM (median 5.81) for random negatives, providing clear separation between the two classes.}
        \label{fig:hbpm-random}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/harmonic_bpm_hard.png}
        \caption{Hard negative sampling. The relationship inverts: hard negatives have a mean harmonic distance of 1.85 BPM (median 2.72), smaller than the 4.90 BPM mean of positive pairs. Because hard negatives are constrained within $\pm 5$ BPM of a positive anchor, they end up closer in tempo than the positive pairs themselves.}
        \label{fig:hbpm-hard}
    \end{subfigure}
...
Results from the survey show an overall satisfaction of the model of $67.84\%$. As it can be seen in \Cref{ScoreHV}, the song which received the highest score is \textit{Twelve - Psyk Rework}, which is coherent as from \Cref{genre} we can see that its genre is Tech House and the dataset used to train the model is mainly Tech House dominated. On the other hand, the song with the lowest score is \textit{Music Sounds Better With You - Stardust} which is a Funky House song and our dataset did not have any Funky House, therefore the prediction is less accurate. The remaining three songs in \Cref{ScoreHV} have somewhat similar scores oscillating between $3.26$ and $3.56$ on the 0--5 scale, as in fact, their genres represent in between $6.9\%$ and $18.7\%$ of the dataset. It can be notice that the score of a song increases as the percentage of its genre in the dataset increases, which is expected as it means the model will find more similar songs to the query. 
...
\begin{figure}[H]
  ...
  \begin{minipage}[c]{0.34\linewidth}
    \caption{Per-query, per-rank mean survey rating for the five queries in \Cref{genre}, on the original 0--5 scale averaged across raters. The right-hand strip gives the per-query mean across ranks; the bottom strip gives the per-rank mean across queries. Overall mean rating across all cells is $3.39$ (equivalent to $67.84\%$ satisfaction). The Rank-5 cell for \textit{No Distance} is missing because that recommendation was not collected in the survey.}
    \label{ScoreHV}
  \end{minipage}
\end{figure}
