...
\begin{figure}[H]
  ...
  \begin{minipage}[c]{0.56\linewidth}
    \caption{Retrieval performance on the held-out test corpus. For each of the 741 (query, true-next-track) test pairs, we score the query against all 1042 test-corpus candidates with the gradient-boosting pair classifier, mask the query itself, and rank by posterior. Hit@$k$ is the share of pairs for which the true next-track lands in the top $k$ of that ranking. The dashed line is the random-pick baseline $k / 1042$, the chance that a uniformly random ranker places the truth in its top $k$, with the shaded band showing its 95\% Wald CI under run-to-run variability at $n = 741$. The model curve sits $5.7\times$ to $9.8\times$ above chance across $k \in [1, 50]$, with the largest relative lift at small $k$.}
    \label{fig:hit-at-k}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.66\linewidth}
    ...
    \caption{Per-query, per-rank mean survey rating on the original 0--5 scale averaged across raters. Right-hand strip is the per-query mean across ranks; bottom strip is the per-rank mean across queries. Overall mean across all cells is $3.39$ ($67.84\%$ satisfaction). The Rank-5 cell for \textit{No Distance} was not collected.}
    \label{ScoreHV}
  \end{minipage}%
  \hfill
  \begin{minipage}[t]{0.31\linewidth}
    ...
    \footnotesize
    \raggedright
    \textit{Elsa - Johann Stone}\\
    \hspace*{1em}\textcolor{gray}{Progressive Trance}\\[3pt]
    \textit{Twelve - Psyk Rework}\\
    \hspace*{1em}\textcolor{gray}{Tech House}\\[3pt]
    \textit{Peace Keeper - Moving Fusion}\\
    \hspace*{1em}\textcolor{gray}{Drum \& Bass}\\[3pt]
    \textit{No Distance - Dixon, Guy Gerber}\\
    \hspace*{1em}\textcolor{gray}{Progressive House}\\[3pt]
    \textit{Music Sounds Better With You - Stardust}\\
    \hspace*{1em}\textcolor{gray}{Funky House}
    \captionof{table}{The five survey query tracks and their genres.}
    \label{genre}
  \end{minipage}
\end{figure}
